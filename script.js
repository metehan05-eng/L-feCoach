...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const updateMessageCounter = () => {
        ...
        dom.messageCounter.innerHTML = `
            <div class="message-limit-info">
                <span>📝 Kalan mesaj hakkı: ${remaining}/10</span>
            </div>
        `;
    };
    ...
    const showLimitReachedCard = () => {
        ...
        card.innerHTML = `
            <div class="limit-card-content">
                <h3>⏰ Konuşma süresi doldu</h3>
                <p>Günlük 10 mesaj hakkınızı kullandınız. Sınırsız konuşma için Premium'a geçin!</p>
                <div class="cooldown-timer">
                    <span id="cooldown-display">5 saat sonra aktifleşir</span>
                </div>
                <button id="upgrade-premium-btn" class="btn btn-primary">Premium'a Geç</button>
            </div>
        `;
    ...
    const updateCooldownTimer = () => {
        ...
        const updateTimer = () => {
            ...
            if (timeLeft <= 0) {
                display.textContent = 'Şimdi aktif!';
                state.isLimitReached = false;
            ...
            display.textContent = `${hours} saat ${minutes} dakika sonra aktifleşir`;
    ...
    const loadChatHistory = () => {
        ...
        appendMessage('Merhaba! Bugün sana nasıl yardımcı olabilirim?', 'ai');
    };
    ...
    const handleSendMessage = async () => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                const errorMsg = data.detail || data.response || 'Bir hata oluştu. Lütfen tekrar deneyin.';
                appendMessage(errorMsg, 'ai');
            }
        } catch (error) {
            console.warn('Backend erişilemedi', error);
            appendMessage('Bağlantı hatası. Lütfen tekrar deneyin.', 'ai');
        } finally {
    ...
    const loadJournalEntries = () => {
        if (!state.token) return;
        dom.journalEntries.innerHTML = '<p>Günlük girişleriniz yükleniyor...</p>';
        ...
        .then(entries => {
            if (entries.length === 0) {
                dom.journalEntries.innerHTML = '<p>Henüz günlük girişi yapmadınız. Yeni bir giriş oluşturmak için yukarıdaki butona tıklayın.</p>';
            } else {
        ...
        .catch(err => {
            ...
            dom.journalEntries.innerHTML = '<p>Günlük girişleri yüklenirken hata oluştu.</p>';
        });
    };
    ...
    const loadGoals = () => {
        if (!state.token) return;
        dom.goalList.innerHTML = '<p>Hedefleriniz yükleniyor...</p>';
        ...
        .then(goals => {
            if (goals.length === 0) {
                dom.goalList.innerHTML = '<p>Henüz hedef belirlemediniz. Yeni bir hedef oluşturmak için yukarıdaki butona tıklayın.</p>';
            } else {
                dom.goalList.innerHTML = goals.map(goal => `
                    <div class="goal-item">
                        ...
                        <small>%${goal.progress} Tamamlandı</small>
                    </div>
                `).join('');
            }
        })
        .catch(err => {
            ...
            dom.goalList.innerHTML = '<p>Hedefler yüklenirken hata oluştu.</p>';
        });
    };
    ...
    const resetInactivityTimer = () => {
        ...
        state.inactivityTimer = setTimeout(() => {
            if (state.currentFeature === 'chat') {
                appendMessage("Bir süredir sessizsin. Dilersen, üzerinde düşündüğün bir konuyu derinlemesine ele alabiliriz.", "ai");
            }
    ...
    dom.newJournalBtn.addEventListener('click', () => {
        ...
        showToast('Yeni günlük girişi özelliği yakında gelecek!', 3000);
    });

    // Goal new goal button
    dom.newGoalBtn.addEventListener('click', () => {
        // TODO: Open new goal modal/form
        showToast('Yeni hedef ekleme özelliği yakında gelecek!', 3000);
    });
        ...
        try {
            ...
            if (response.ok && data.access_token) {
                ...
                try {
                    ...
                    showToast('Giriş başarılı — token konsola yazıldı ve kaydedildi.', 4500);
                } catch (e) {
                    console.warn('Token debug failed', e);
                }
            } else {
                showAuthError(data.detail || 'Hata oluştu');
            }
        } catch (err) {
            showAuthError('Bağlantı hatası');
        }
    });
